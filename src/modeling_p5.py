...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        # Prepare per-layer head mask without relying on removed HF helpers.
        layer_head_masks = [None] * len(self.block)
        if head_mask is not None:
            if isinstance(head_mask, (list, tuple)):
                for i, m in enumerate(head_mask[: len(self.block)]):
                    layer_head_masks[i] = m
            elif torch.is_tensor(head_mask):
                if head_mask.dim() == 1:
                    layer_head_masks = [head_mask] * len(self.block)
                else:
                    for i in range(min(head_mask.size(0), len(self.block))):
                        layer_head_masks[i] = head_mask[i]
        ...
        if self.config.num_layers > 0:
            ...
            for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
                ...
                if layer_head_masks[i] is not None:
                    layer_kwargs["head_mask"] = layer_head_masks[i]

                # HF signatures changed across versions (`head_mask`, `past_key_values`).
                try:
                    layer_outputs = layer_module(hidden_states, **layer_kwargs)
                except TypeError as e:
                    msg = str(e)
                    if "head_mask" in msg:
                        layer_kwargs.pop("head_mask", None)
                        layer_outputs = layer_module(hidden_states, **layer_kwargs)
                    elif "past_key_values" in msg:
                        layer_kwargs["past_key_value"] = layer_kwargs.pop("past_key_values")
                        layer_outputs = layer_module(hidden_states, **layer_kwargs)
                    else:
                        raise
                ...
                # HF changed T5Block outputs across versions. Pick the first 4D tensor
                # after hidden_states as the self-attention position bias.
                position_bias = None
                for output in layer_outputs[1:]:
                    if torch.is_tensor(output) and output.dim() == 4:
                        position_bias = output
                        break
                if position_bias is None and len(layer_outputs) > 1:
                    # Fallback for older layouts.
                    position_bias = layer_outputs[1]

                # append next layer key value states
                if use_cache and present_key_value_state is not None:
...
class P5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        # Keep compatibility with newer transformers initialization/tied-weights flow.
        self.post_init()
